...
describe 'ActiveRecord Obstacle Course' do
  ...
  it 'finds order id of smallest order' do
    ...
    # ------------------ Using ActiveRecord ----------------------
    # order_id = Order.select(:id).order(:amount).first.id ---works but easier way
    order_id = Order.order(amount: :asc).limit(1).first.id
    #                           limit(1) AR fetches 1 row, "first" is ruby...most efficient
    # ------------------------------------------------------------
  ...
  it 'returns the names of users who ordered one specific item' do
    ...
    # ------------------ Using ActiveRecord ----------------------
    # ids = Order.joins(:items).group("item_id = 8").pluck(:user_id)
    # users = User.find(ids).pluck(:name)
  ...
  it 'returns all orders for user 2 which include item_4' do
    ...
    # ------------------ Improved Solution ----------------------
    orders = Order.joins(:items).where('user_id=? AND items.id=?',2,4)
    # -----------------------------------------------------------
  ...
  it 'returns a table of information for all users items' do
    ...
    # ------------------ ActiveRecord Solution ----------------------
    custom_results =
    User.select("users.name as name, count(items.id) as total_item_count").
    joins(:items).
    group(:name).
    order(name: :desc)
    # User.select("users.name as name, count(items.id) as total_item_count").
  ...
  it 'returns a table of information for all users orders and item counts' do
    ...
    # joins('join orders ON orders.user_id=users.id').
    # joins('join order_items ON order_items.order_id=orders.id').
    # group('orders.id AND users.name').
